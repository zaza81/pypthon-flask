...
class TestDate(unittest.TestCase):
    ...
    def compareDay(self, input, target):
        service = DateService()
        result = service.extractDay(input)
        self.assertEqual(result.month, target.month)
        self.assertEqual(result.day, target.day)

    def compareDate(self, input, target, TIME=False):
        service = DateService()
    ...
    def compareDays(self, input, targets):
        service = DateService()
        results = service.extractDays(input)
        for (result, target) in zip(results, targets):
            self.assertEqual(result.month, target.month)
            self.assertEqual(result.day, target.day)

    def compareDates(self, input, targets, TIME=False):
        service = DateService()
    ...
    def testTwoDates(self):
        ...
        self.compareDates(input, targets, TIME=True)

    #
    # Single extractor tests
    #
    def testExtractDay(self):
        input = "On March 13 at 12:30pm."
        target = datetime.datetime(2014, 3, 13)
        self.compareDay(input, target)
